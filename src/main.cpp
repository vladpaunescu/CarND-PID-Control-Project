...
int main() {
	...
	double Kp = 0.13, Ki = 0.02, Kd = 0.005;
	...
	h.onMessage(
			[&pid, &cur_time, &prev_time](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
				...
				if (length && length > 2 && data[0] == '4' && data[1] == '2')
				{
					...
					if (s != "") {
						...
						if (event == "telemetry") {
							...
							double throttle_value = 0.55 - 1.5 * pid.TotalError() ;
							...
							std::cout << "PID error: " << pid.TotalError() << std::endl;
